...
def face_detect(frame): 
    ...
    names = [] 
    # loop over the facial embeddings for encoding in encodings: 
    ...
    if True in matches: 
        ...
        for i in matchedIdxs: name = data["names"][i] 
            ...
            if current_name != name: 
                current_name = name print(current_name) 
                ...
                for ((top, right, bottom, left), name) in zip(boxes, names): 
                    ...
                    y = top - 15 
                    if top - 15 > 15 else top + 15 cv2.putText(frame, name, (left, y), cv2.FONT_HERSHEY_SIMPLEX, .8, (0, 255, 255), 2) 
def sendMail(): 
    ...
    msg = EmailMessage() 
    # Set the email body 
    body ="[WARNING] there are strangers"
